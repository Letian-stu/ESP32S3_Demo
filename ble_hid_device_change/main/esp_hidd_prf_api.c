...
void esp_hidd_send_my_control_value(uint16_t conn_id, uint8_t key_cmd)
{
    ESP_LOGD(HID_LE_PRF_TAG, "key_cmd = %d",key_cmd);
    ESP_LOGI(HID_LE_PRF_TAG, "key_cmd = %d",key_cmd);
    hid_dev_send_report(hidd_le_env.gatt_if, conn_id,
                        HID_RPT_ID_CONTROL_IN, HID_REPORT_TYPE_INPUT, sizeof(key_cmd), &key_cmd);
    return;
}
#endif
